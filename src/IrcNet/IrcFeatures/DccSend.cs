...
#pragma warning disable IDE0060 // API
        internal DccSend(IrcFeatures irc, IPAddress externalIpAdress, CtcpEventArgs e) : base()
        {
            ...
            if (e.Data.MessageArray.Length > 4)
            {
                ...
                var okPo = int.TryParse(e.Data.MessageArray[4], out var port);  // port 0 = passive
                if (e.Data.MessageArray.Length > 5)
                {
                    _ = long.TryParse(FilterMarker(e.Data.MessageArray[5]), out filesize);
                    _Filesize = filesize;
